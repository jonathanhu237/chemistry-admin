# Chunk 5 Evidence-First 全量终审 Prompt

你负责 `chunk_5`，只处理 chunk 5，不修改其他 chunk，不修改 OpenSpec tasks。
...
- 输出报告: `E:/chemistry-admin/artifacts/point-aware-question-bank/reviewed_old_bank_chunks/chunk_5_evidence_first_final_report.md`
- 实验范围: `20-2-08` 至 `20-3-14`
- 对应 OpenSpec 任务: `7.1` 至 `7.18`

## 必须比对的证据来源

- 最终有效题: 当前 release JSON 中每道 active effective question。
- 原题/初审/旧改写参考: `chunk_5_reviewed_v1.json`, `chunk_5_semantic_final_v1.json`, `release_input_freeze_v1/chunk_5_polished_final_v1.json`, `prompts/chunk_5_review_prompt.md`。
...
- 修改后的 `chunk_5_release_final_v1.json`，且 UTF-8 JSON parse 通过。
- 更新后的 `chunk_5_manual_semantic_rereview_log.md`，覆盖 chunk 5 每道 active 题一次。
- 新建/更新 `chunk_5_evidence_first_final_report.md`，包含：
  ...
  - option_link 修复/异常列表。
  - JSON parse 校验结果。
  - 明确说明：chunk 5 已按 evidence-first 标准逐题全量终审，未用抽检替代全量检查。
