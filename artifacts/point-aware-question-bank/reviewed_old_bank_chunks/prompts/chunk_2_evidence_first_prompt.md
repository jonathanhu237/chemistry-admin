# Chunk 2 Evidence-First 全量终审 Prompt

你负责 `chunk_2`，只处理 chunk 2，不修改其他 chunk，不修改 OpenSpec tasks。

## 范围

- Release JSON: `E:/chemistry-admin/artifacts/point-aware-question-bank/reviewed_old_bank_chunks/chunk_2_release_final_v1.json`
- Manual log: `E:/chemistry-admin/artifacts/point-aware-question-bank/reviewed_old_bank_chunks/manual_semantic_rereview_logs/chunk_2_manual_semantic_rereview_log.md`
- 输出报告: `E:/chemistry-admin/artifacts/point-aware-question-bank/reviewed_old_bank_chunks/chunk_2_evidence_first_final_report.md`
- 实验范围: `19-3-03` 至 `19-6-01`
- 对应 OpenSpec 任务: `4.1` 至 `4.16`

## 必须比对的证据来源

- 最终有效题: 当前 release JSON 中每道 active effective question。
- 原题/初审/旧改写参考: `chunk_2_reviewed_v1.json`, `chunk_2_semantic_final_v1.json`, `release_input_freeze_v1/chunk_2_polished_final_v1.json`, `prompts/chunk_2_review_prompt.md`。
...
- 修改后的 `chunk_2_release_final_v1.json`，且 UTF-8 JSON parse 通过。
- 更新后的 `chunk_2_manual_semantic_rereview_log.md`，覆盖 chunk 2 每道 active 题一次。
- 新建/更新 `chunk_2_evidence_first_final_report.md`，包含：
  ...
  - option_link 修复/异常列表。
  - JSON parse 校验结果。
  - 明确说明：chunk 2 已按 evidence-first 标准逐题全量终审，未用抽检替代全量检查。
