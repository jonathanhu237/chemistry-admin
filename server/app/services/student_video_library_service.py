...
def _build_documents(experiments: list[dict[str, Any]], profiles: list[dict[str, Any]]) -> list[VideoLibraryDocument]:
    ...
    def search(self, query: str, documents: list[VideoLibraryDocument], limit: int) -> list[VideoLibraryDocument]:
        ...
        payload = {
            ...
            "query": {
                "multi_match": {
                    "query": query,
                    "fields": ["title^4", "subtitle^2", "snippet^2", "search_text"],
                    "type": "best_fields",
                }
            },
        }
...
def search_student_video_library(user: AuthUser, *, query: str = "", limit: int = 24) -> StudentVideoLibrarySearchResponse:
    settings = get_settings()
    with db_session() as session:
        experiments = _load_published_experiments(session)
    profiles = _learning_profiles()
    documents = _build_documents(experiments, profiles)
...
def video_library_index_preview(user: AuthUser) -> dict[str, Any]:
    with db_session() as session:
        experiments = _load_published_experiments(session)
    profiles = _learning_profiles()
    documents = _build_documents(experiments, profiles)
